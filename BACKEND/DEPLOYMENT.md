# =============================================================================
# FRAUD DETECTOR - DEPLOYMENT GUIDE
# Local dev (shared infra) + production (hokeka.com)
# =============================================================================

## Local Dev — Run Against Shared Infrastructure (HOK-130 / HOK-133)

The Spring Boot backend in this directory is the canonical AML API server. For
local development it must run against the shared infrastructure compose at
`architecture-v2/infra/docker-compose.infrastructure.yml` (Postgres, Redis,
Kafka, Zookeeper, Aerospike). Do NOT stand up parallel infra.

### One-time setup

```bash
# 1. Start shared infrastructure (from repo root)
docker compose -f architecture-v2/infra/docker-compose.infrastructure.yml up -d

# 2. Wait for healthchecks (postgres, kafka, aerospike all healthy)
docker compose -f architecture-v2/infra/docker-compose.infrastructure.yml ps
```

### Run command

The shared compose creates a Postgres database `fraud_detector` with user
`aml_user` / password `changeme` (override via env). The backend's
`application.properties` defaults expect different credentials, so the run
command must set the database overrides explicitly:

```bash
cd BACKEND

DATABASE_URL=jdbc:postgresql://localhost:5432/fraud_detector \
DATABASE_USERNAME=aml_user \
DATABASE_PASSWORD=changeme \
KAFKA_BOOTSTRAP_SERVERS=localhost:9092 \
AEROSPIKE_HOSTS=localhost:3000 \
...
The service binds on `http://localhost:2637/api/v1` (context path is
`/api/v1`). Swagger is at `http://localhost:2637/api-docs/swagger-ui.html`.

### Smoke check

```bash
curl -fsS http://localhost:2637/actuator/health
curl -fsS http://localhost:2637/api/v1/auth/me   # returns 401 unauthenticated
```

### CBK outbound reporting (HOK-130 requirement)

Outbound CBK report submissions originate from this backend (never frontend or
PSP code paths). Wire it via `cbk.reporting.*` properties:

```bash
CBK_REPORTING_BASE_URL=https://cbk-reporting.example.org \
CBK_REPORTING_API_KEY=<key> \
CBK_REPORTING_SUBMIT_PATH=/v1/reports \
mvn spring-boot:run
```

When `CBK_REPORTING_BASE_URL` is empty, `CbkReportService.submitReport` still
persists the submission in `regulatory_submissions` but leaves the status as
`PENDING_REVIEW` (no outbound HTTP call). When set, the gateway POSTs the
report and transitions the submission to `FILED` (with the regulator's
acknowledgement reference) or `REJECTED` (with the failure reason).

The gateway implementation lives in
`src/main/java/com/posgateway/aml/service/reporting/CbkExternalGateway.java`.

---

## Production Deployment — External Access via hokeka.com
