 ...
 */
@Service
public class JevBranchEvaluator {
    ...
    private final JevBandsService bandsService;

    public JevBranchEvaluator(JevBandsService bandsService) {
        this.bandsService = bandsService;
    }

    public EvaluationResult evaluate(JevDecisionPoint decisionPoint,
                                     Long pspId,
    ...
    private EvaluationResult evaluateDp1(Map<String, JsonNode> answers,
        ...
        if (laundering >= tUp || highOrSevere >= tUp2 || "mlro_direct".equals(owningQueue)) {
            return new EvaluationResult(JevBranch.ESCALATE_UP, thresholds, diagnostics);
        }
        ...
        if (laundering < tLow
            ...
            return new EvaluationResult(JevBranch.LOW_RISK_CANDIDATE, thresholds, diagnostics);
        }
        return new EvaluationResult(JevBranch.DEFAULT, thresholds, diagnostics);
    }
    ...
    private EvaluationResult evaluateDp2(Map<String, JsonNode> answers, ThresholdSet thresholds) {
        ...
        if (sameEntity < tFp && "contradicted".equals(matchBasis)) {
            return new EvaluationResult(JevBranch.LIKELY_FALSE_POSITIVE, thresholds, diagnostics);
        }
        return new EvaluationResult(JevBranch.DEFAULT, thresholds, diagnostics);
    }
    ...
    private EvaluationResult evaluateDp3(Map<String, JsonNode> answers, ThresholdSet thresholds) {
        ...
        if ("close_no_suspicion".equals(nextStep) && strGrounds < tClose) {
            return new EvaluationResult(JevBranch.PROPOSE_CLOSURE, thresholds, diagnostics);
        }
        return new EvaluationResult(JevBranch.DEFAULT, thresholds, diagnostics);
    }
    ...
    private EvaluationResult evaluateDp4(Map<String, JsonNode> answers,
        ...
        int jevTier = dominantScoreBucket(answers, "risk_tier");
        diagnostics.put("deterministic_risk_tier", deterministicTier);
        diagnostics.put("jev_risk_tier_bucket", jevTier);

        if ((jevTier > deterministicTier && highTierMass >= tTier) || eddRequired >= tEdd) {
            return new EvaluationResult(JevBranch.RAISE_RISK_TIER, thresholds, diagnostics);
        }
        return new EvaluationResult(JevBranch.DEFAULT, thresholds, diagnostics);
    }
    ...
    private EvaluationResult evaluateDp5(Map<String, JsonNode> answers, ThresholdSet thresholds) {
        ...
        return new EvaluationResult(JevBranch.ESCALATE_HUMAN, thresholds, diagnostics);
    }
    ...
    private static boolean hasHardRegulatoryFlag(JevDecisionContext context) {
    ...
    private static int deterministicTier(JevDecisionContext context) {
        Map<String, Object> features = context.getFeatures();
        if (features == null) {
            return 0;
        }
