...
const FORBIDDEN = /openrouter|\bjev\b|\blaya\b|typesafe|anthropic|openai|\bprompt\b|\btoken\b|\bcost\b|gpt-/i;
...
describe("PSP-facing AI disclosure guards", () => {
  it("AiVerdictPanel uses generic Hokeka branding and hides vendor fields from tenants", () => {
    ...
    expect(source).not.toMatch(/\bLaya\b/i);
  ...
  it("settings tabs hide Hokeka AI operator tab from PSP roles", () => {
    const source = readFileSync(resolve(__dirname, "settingsTabs.ts"), "utf8");
    expect(source).toContain('"Hokeka AI"');
