...
/**
 * Versioned prompt templates stored under {@code classpath:hokeka-ai/prompts/}.
 */
@Service
public class AiPromptTemplateService {
    ...
    public String resolveSystemPrompt(AiEngineType engine, String version) {
        String key = engine.name() + "/" + version;
        return cache.computeIfAbsent(key, k -> loadTemplate(engine, version));
    }

    private String loadTemplate(AiEngineType engine, String version) {
        String path = "hokeka-ai/prompts/" + engine.name().toLowerCase() + "/" + version + ".txt";
        try {
    ...
    private static String defaultPrompt(AiEngineType engine) {
        return switch (engine) {
            case TRANSACTION_RISK -> """
                    You are Hokeka AI, Hokeka's AML transaction risk advisor. Review borderline transaction
                    ...
                    """;
            case ALERT_TRIAGE -> """
                    You are Hokeka AI, Hokeka's alert triage assistant. Prioritize alerts for analysts:
                    suggest priority, summary, and recommended action. Output is advisory only.
                    """;
            case CASE_TRIAGE -> """
                    You are Hokeka AI, Hokeka's case triage assistant. Summarize case context and recommend
                    next investigative steps. Output is advisory — analysts decide outcomes.
                    """;
            case SANCTIONS_DISAMBIGUATION -> """
                    You are Hokeka AI, Hokeka's sanctions/PEP match disambiguation assistant. Assess whether
                    ...
                    """;
            case KYC_EDD -> """
                    You are Hokeka AI, Hokeka's KYC/EDD review assistant. Summarize identity evidence gaps
                    and recommend enhanced due diligence steps. Advisory only — no auto-approval.
                    """;
            case G2_CONTENT -> """
                    You are Hokeka AI, Hokeka's merchant website/content monitoring assistant. Assess URL/MCC/
                    site-scan signals for transaction laundering or undeclared business lines.
                    """;
            case ADVERSE_MEDIA -> """
                    You are Hokeka AI, Hokeka's adverse media relevance scorer. Rate GDELT/article relevance
                    to the subject entity for AML investigation. Advisory only.
                    """;
            case RULE_SUGGESTION -> """
                    You are Hokeka AI, Hokeka's AML rule suggestion assistant. Generate ONE SpEL rule definition
                    ...
                    """;
            case FRAUD_SCORING -> """
                    You are Hokeka AI, Hokeka's fraud scoring advisor. Interpret ML/rule signals for post-scoring
                    review when deterministic outcome is borderline. Advisory only.
                    """;
            case SAR_NARRATIVE_VERIFICATION -> """
                    Typed decisions handle SAR narrative verification; this prompt is unused.
                    """;
        };
    }
}
